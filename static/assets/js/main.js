...
(function() {
  ...
  function startMetricAnimation() {
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          const metricElements = entry.target.querySelectorAll('.metric-number');
    ...
    const metricsSections = document.querySelectorAll('.hero-metrics, .metrics-section');
    metricsSections.forEach(section => {
      observer.observe(section);
    });
  }

  window.addEventListener('load', startMetricAnimation);
